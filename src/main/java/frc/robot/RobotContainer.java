...
public class RobotContainer {
  ...
  // public final CommandXboxController m_driverController = new
  // CommandXboxController(0);

  // Replace with CommandPS4Controller or CommandJoystick if needed
  // final CommandXboxController drivebase.driverXbox = new
  // CommandXboxController(0);
  ...
  SwerveInputStream driveAngularVelocity = SwerveInputStream.of(drivebase.getSwerveDrive(),
      () -> drivebase.ySpeed,
      () -> drivebase.xSpeed )
      .withControllerRotationAxis(() -> drivebase.rotspeed )
      .deadband(.15)
...
//       .headingWhile(true);
  ...
  private void configureBindings() {
    ...
    m_operatorController.button(8).onTrue(m_elevator.m_elbow.Human());
     drivebase.driverXbox.y().onTrue(m_wrist.upsidedown());
    // drivebase.driverXbox.b().whileTrue(m_climber.up());
    // *******home is in robot.java**********

    // m_operatorController.button(2).onFalse(drivebase.driveToPose(new Pose2d(new
    // Translation2d(3.177, 4.167), Rotation2d.fromDegrees(0))));
    ...
    // } else
    // {
    // if (drivebase.driverXbox.getLeftTriggerAxis()<.1 &&
    // drivebase.driverXbox.getRightTriggerAxis()<.1)
    // {
    drivebase.setDefaultCommand(driveFieldOrientedAnglularVelocity);
    // }

    if (Robot.isSimulation()) {
      // drivebase.driverXbox.start().onTrue(Commands.runOnce(() ->
